...
fn parse_key(keyspec: &str) -> Result<Key> {
    if keyspec.len() != 5 || keyspec.chars().count() != 5 {
        ...
        let part = (if keyspec.chars().nth(4).unwrap() == 'a' { Ok(Part::One) }
            else if keyspec.chars().nth(4).unwrap() == 'b' { Ok(Part::Two) }
            else { Err(anyhow!("final character of keyspec must be 'a' or 'b'")) })?;

        Ok(Key { event: Event::try_from(year)?, day: Day::try_from(day)?, part })
    }
}